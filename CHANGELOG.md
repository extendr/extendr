...
- **Breaking**: `metadata::Func` no longer has a `func_ptr` field and `register_call_methods` takes `&[CallMethod]`. Wrapper registration pointers are now emitted via generated `r_init__*` functions, so the metadata/wrapper-generation path no longer references R's C API and the build-time `document` helper generates wrappers without linking `-lR`. On macOS/Linux the unused R-calling code is dead-stripped; on windows-gnu (where it shares a single `.text` section per codegen unit and cannot be stripped) the never-called R symbols are left unresolved via `--noinhibit-exec`. <https://github.com/extendr/extendr/pull/1102>
