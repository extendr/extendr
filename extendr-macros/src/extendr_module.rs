...
pub fn extendr_module(item: TokenStream) -> TokenStream {
    ...
    TokenStream::from(quote! {
        ...
        pub extern "C" fn #write_make_module_wrappers(
            ...
            let body = match #module_metadata_name().make_r_wrappers(true, pkg) {
                Ok(s) => s,
                Err(e) => {
                    eprintln!("extendr: make_r_wrappers failed for package '{}': {}", pkg, e);
                    return 3;
                }
            };
            let contents = format!(
            ...
            match std::fs::write(path, contents) {
                Ok(_) => 0,
                Err(e) => {
                    eprintln!("extendr: failed to write '{}': {}", path, e);
                    1
                }
            }
        }
