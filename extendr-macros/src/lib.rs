...
/// Derives an implementation of `TryFrom<List> for Struct` and `TryFrom<&List> for Struct` on this struct.
///
...
/// ```
///
/// See [`IntoList`] for converting arbitrary Rust types into R type by using
/// R's list / `List`.
///
...
/// Derives an implementation of `From<Struct> for List` and `From<&Struct> for List` on this struct.
///
...
/// # Details
///
/// Note, the `From<Struct> for List` behaviour is different from what is obtained by applying the standard `#[extendr]` macro
