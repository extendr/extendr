...
#[test]
fn test_doubles_from_altrep_values() {
    test! {
        let vec = Doubles::from_altrep_values((0..1000000000).map(|x| x as f64));
...
mod num_complex {
    ...
    fn from_altrep_values() {
        test! {
            let vec = Complexes::from_altrep_values((0..1000000000).map(|x| x as f64));
            assert_eq!(vec.is_altrep(), true);
