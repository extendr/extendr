...
pub unsafe fn register_call_method(info: * mut libR_sys::DllInfo, func: unsafe extern "C" fn() -> *mut ::std::os::raw::c_void, wrap_name_str: &str, num_args: i32) {
    let call_symbol = std::ffi::CString::new(wrap_name_str).unwrap();
    eprintln!("register {} {:x} {}", wrap_name_str, std::mem::transmute::<_, usize>(func), num_args);
    let call_methods : [libR_sys::R_CallMethodDef; 2] = [
        libR_sys::R_CallMethodDef {
            name: call_symbol.as_ptr(),
            fun: Some(func),
            numArgs: num_args
        },
        libR_sys::R_CallMethodDef {
...
mod tests {
    ...
    fn export_test() {
        ...
        unsafe {
            wrap__inttypes(Robj::from(1).get(), Robj::from(2).get(), Robj::from(3).get(), Robj::from(4).get(), Robj::from(5).get(), Robj::from(6).get(), Robj::from(7).get(), Robj::from(8).get());
            wrap__inttypes(Robj::from(1.).get(), Robj::from(2.).get(), Robj::from(3.).get(), Robj::from(4.).get(), Robj::from(5.).get(), Robj::from(6.).get(), Robj::from(7.).get(), Robj::from(8.).get());
            wrap__floattypes(Robj::from(1.).get(), Robj::from(2.).get());
            wrap__floattypes(Robj::from(1).get(), Robj::from(2).get());
            wrap__strtypes(Robj::from("abc").get(), Robj::from("def").get());
            wrap__vectortypes(Robj::from(&[1, 2, 3] as &[i32]).get(), Robj::from(&[4., 5., 6.] as &[f64]).get());
