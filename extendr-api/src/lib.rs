...
#[doc(hidden)]
pub unsafe fn register_call_methods(info: *mut extendr_ffi::DllInfo, methods: &[CallMethod]) {
    let mut rmethods = Vec::with_capacity(methods.len() + 1);
    for method in methods {
        make_method_def(&mut rmethods, method);
    }
