...
use arrow2::ffi::*;
use polars::prelude::{ArrowField, DataFrame, df, NamedFrom, Series};
...
impl TryFrom<DataFrame> for Robj {
    ...
    fn try_from(value: DataFrame) -> crate::Result<Self> {
        ...
        if pkg.is_empty(){
            return Err(Error::Other("The arrow package needs to be installed".into()))
        }
        ...
        for series in value.get_columns() {
            ...
            let imported_arr = call!("arrow::Array$import_from_c", array_ptr as usize, schema_ptr as usize)?;
