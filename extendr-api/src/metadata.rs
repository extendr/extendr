...
fn write_function_wrapper(w: &mut Vec<u8>, func: &Func, package_name: &str, use_symbols: bool) -> std::io::Result<()> {
    if func.hidden {
        return Ok(());
    }
...
fn write_method_wrapper(w: &mut Vec<u8>, func: &Func, package_name: &str, use_symbols: bool, class_name: &str) -> std::io::Result<()> {
    if func.hidden {
        return Ok(());
    }
    ...
    let formal_args = if !actual_args.is_empty() && actual_args[0] == "self" {
        ...
        func.args.iter().skip(1).map(|arg| arg.name).collect::<Vec<_>>()
    } else {
        actual_args.clone()
    };
    ...
    if use_symbols {
        ...
        write!(w, "\"wrap__{}__{}\", PACKAGE=\"{}\"", class_name, func.name, package_name)?;
    }

    if actual_args.is_empty() {
        writeln!(w, ")")?;
    } else {
        writeln!(w, ", {})", actual_args)?;
    }
...
impl Metadata {
    pub fn make_r_wrappers(&self, use_symbols: bool) -> std::io::Result<String> {
        let mut w = Vec::new();

        writeln!(w, "#' @name {}", self.name)?;
        writeln!(w, "#' @docType package")?;
        writeln!(w, "#' @useDynLib {}, .registration = TRUE", self.name)?;
        writeln!(w, "NULL")?;
        writeln!(w, "")?;
        writeln!(w, "# Generated by")?;
        writeln!(w, "#")?;
        writeln!(w, "# cat(.Call(\"wrap__make_{}_wrappers\", use_symbols={}))\n", self.name, if use_symbols {"TRUE"} else {"FALSE"})?;
        
        for func in &self.functions {
            write_function_wrapper(&mut w, func, self.name, use_symbols)?;
        }
